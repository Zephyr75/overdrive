...
if [ -z "$slangc" ]; then
    slangc=$here/../cpp/build-vk/_deps/slang-src/bin/slangc
fi
if [ ! -x "$slangc" ]; then
    echo "slangc not found. Put it on PATH or set SLANGC=/path/to/slangc." >&2
    echo "Prebuilt SDKs: https://github.com/shader-slang/slang/releases" >&2
    exit 1
fi
