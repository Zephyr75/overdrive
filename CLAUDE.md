...
Overdrive is a Go game engine whose graphics layer runs on **both OpenGL 4.1 and Vulkan 1.3** from one shader set, picked at startup by the TOML settings file (`src/configs/opengl.toml` by default). It also carries an ECS, Verlet physics, a gutter-based UI overlay and a Blender XML export plugin.
...
go build ./...
go test ./...        # std140 layout + settings + showcase-scene checks; no GPU needed
go run .             # reads configs/opengl.toml

go run . -config configs/opengl.toml   # OpenGL 4.1 core, 4x MSAA
go run . -config configs/vulkan.toml   # Vulkan 1.3, same settings otherwise
OVERDRIVE_VK_VALIDATION=1 go run . -config configs/vulkan.toml
...
3. **Uniforms are two typed structs, split by update frequency.** `renderer.FrameUniforms` (1280 bytes, camera/lights/shadow maps) is published once per pass by `BindFrameUniforms`; `renderer.DrawUniforms` (128 bytes, model matrix + material) goes out per draw. Both mirror `shaders/slang/common.slang` field for field, and **both backends upload them by memcpy** — Vulkan into a ring buffer plus two pushed device addresses, OpenGL into two std140 UBOs (binding points 0 and 1). That works only because of the **16-byte cell rule** documented in `common.slang`: every `float3` is followed by a scalar, loose scalars come in fours, no member is a scalar array (`int4`, never `int[4]`). Fill every cell and std140 *is* scalar layout, so no marshalling code exists. Break it and OpenGL silently renders garbage. Guards: an `init()` size panic in `renderer/uniforms.go`, and `opengl/uniforms_test.go`, which re-derives std140 from the generated GLSL and diffs every member against `unsafe.Offsetof`. **After editing `common.slang`, rebuild shaders and run `go test ./opengl/`.**
