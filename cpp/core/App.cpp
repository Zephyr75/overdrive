...
void App::run(const std::string& scenePath) {
    ...
    Shader forwardShader   ("shaders/forward.vert.glsl",    "shaders/forward.frag.glsl");
    Shader depthShader     ("shaders/depth.vert.glsl",      "shaders/depth.frag.glsl");
    Shader depthCubeShader ("shaders/depth_cube.vert.glsl", "shaders/depth_cube.frag.glsl",
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        for (auto& light : scene.lights) {
            glm::mat4 mat = light.renderLight(nearPlane, farPlane,
                                              depthShader, depthCubeShader, scene);
