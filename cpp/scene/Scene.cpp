...
Scene::Scene(const std::string& xmlPath) {
    ...
    if (!result) {
        std::cerr << "Failed to load scene: " << xmlPath << " — " << result.description() << "\n";
        return;
    }
...
void Scene::updateMeshes() {
    for (auto& m : meshes) m.updateVertices();
}

// ---------- accessors --------------------------------------------------------

Mesh* Scene::getMesh(const std::string& n) {
    for (auto& m : meshes) if (m.name == n) return &m;
    return nullptr;
}

Light* Scene::getLight(const std::string& n) {
    for (auto& l : lights) if (l.name == n) return &l;
    return nullptr;
}

// ---------- rendering --------------------------------------------------------

void Scene::renderScene(const Shader& shader, const glm::mat4& lightSpaceMatrix, float farPlane) const {
    shader.use();

    glm::mat4 view = glm::lookAt(camera.pos, camera.pos + camera.front, camera.up);
    glm::mat4 proj = glm::perspective(
        glm::radians(camera.fov), Settings::aspectRatio(), 0.1f, 100.0f);
    glm::mat4 model(1.0f);
