...
void Mesh::load(const std::string& objPath, const std::string& mtlDir, glm::vec3 pos) {
    position        = pos;
    initialPosition = pos;
...
void Mesh::setup() {
    ...
    for (auto& rv : rawVertices) {
        ...
        buf.push_back(rv.normal.x); buf.push_back(rv.normal.y); buf.push_back(rv.normal.z);
        buf.push_back(rv.texcoord.x); buf.push_back(rv.texcoord.y);
    }
    ...
    for (auto& sm : submeshes) {
        ...
        glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                     sm.indices.size() * sizeof(uint32_t),
...
void Mesh::draw(const Shader& shader, const Scene& scene) const {
    ...
    for (auto& sm : submeshes) {
        ...
        const Material& mat = (mi >= 0 && mi < (int)materials.size())
                              ? materials[mi]
                              : Material{};
        ...
        glBindVertexArray(sm.vao);
        glDrawElements(GL_TRIANGLES, static_cast<GLsizei>(sm.indices.size()), GL_UNSIGNED_INT, 0);
        glBindVertexArray(0);
    }
}
