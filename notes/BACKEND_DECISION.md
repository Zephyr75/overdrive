...
**Status: decided. §5 executed 2026-08-05; §5.3 and §9 items 2 onward still open.**
...
`FEATURES.md`, `TODO.md`, `notes/README.md`, and staleness banners on
...
### 5.3 The 16-byte cell rule dies — **not yet done**

**Decided: delete it.** Deliberately left out of the deletion commit: it reorders
live uniform structs, and unlike the rest of §5 a mistake there renders silent
garbage rather than failing to compile. The comments in `renderer/uniforms.go`
and `common.slang` now say the rule is vestigial; the fields have not moved. The rule exists solely because OpenGL 4.1 uniform blocks
must use std140. Vulkan already compiles with `-fvk-use-scalar-layout` and reads
the blocks through a device-address pointer, so scalar layout — which is exactly
Go's packing for `float32`/`int32` structs — is what the shader sees regardless.

What that permits:

- `LightData.Reserved0/1/2` deleted. The three fields exist only because std140
  rounds a struct's array stride to 16 and 17 fields do not divide by 4.
- `float3` no longer needs a scalar behind it. Fields group by meaning.
- Scalar arrays allowed: `int[4]`, not `int4`. `PointShadowLights` becomes an
  ordinary array.
- `opengl/uniforms_test.go` deleted with the package.
- The LAYOUT RULE block in `common.slang` and the long comment above
  `FrameUniforms` collapse to one line: *Go packing is scalar layout; keep the
  Slang struct in the same field order*.

**Keep the `init()` size panic** in `renderer/uniforms.go`. Its constants change
(`LightData` 80 → 68, `FrameUniforms` and `DrawUniforms` follow), and it no
longer guards std140 — but it still catches the one failure mode left: editing
the Go struct or the Slang struct and not the other. Cheap tripwire, no reason
to lose it.

Reorder the structs while doing this. They are currently shaped by a constraint
...
| # | item | days | why |
|---|---|---|---|
| 1 | ~~**Delete OpenGL**~~ (§5) | ✅ | Done 2026-08-05. §5.3, the 16-byte cell rule, is the one piece left |
